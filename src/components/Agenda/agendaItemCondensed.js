...
const AgendaItemCondensed = ({ item }) => {
  return (
    <MyFlex>
      <AgendaItemDate>
        {item.dateFinEvenement
          ? format(new Date(item.dateDebutEvenement), 'eee dd LLL', { locale: fr }) +
            ' au ' +
            format(new Date(item.dateFinEvenement), 'eee dd LLL yyyy', { locale: fr })
          : format(new Date(item.dateDebutEvenement), 'eee dd LLL yyyy', { locale: fr })}
      </AgendaItemDate>
